...
export const GroupDrawArrowSocket = (data: {room: string, info: {fromx: string, fromy: string, tox: string, toy: string, color: string, resolution: number}} ) => {
    socket.emit("group:draw_arrow", data); 
}
export const GroupDrawCircleSocket = (data: {room: string, info: {x: string, y: string, color: string, resolution: number}} ) => {
